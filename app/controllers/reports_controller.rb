class ReportsController < ApplicationController
  def index
    ...
    sql = "select p.name, sum(od.quantity) from order_details as od inner join products as p on p.id = od.product_id where od.finish_date = '#{@current_date}' group by od.product_id;"
