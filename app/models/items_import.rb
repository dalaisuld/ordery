      ...
      (3..spreadsheet.last_row).map do |i|
        ...
        if spreadsheet.row(i)[1] != 0
          ActiveRecord::Base.transaction do
            ...
            order = Order.create(transition_date: transition_date,user_id: user_id, amount: amount, description: description, phone_number: phone_number, account_number: account_number)
            product_id_quantities.each do |product_id_quantity|
              ...
              if product.present? && order.present?
                order_detail = OrderDetail.create(order: order, product: product, quantity: quantity, price: product.price)
